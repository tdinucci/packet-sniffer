...
sockaddr_in WindowsBootstrapper::get_network_iterface()
{
	...
	} while (selected_interface_id < 0 || selected_interface_id > max_interface_id);

	memset(&dest, 0, sizeof(dest));
	memcpy(&dest.sin_addr.s_addr, local->h_addr_list[selected_interface_id], sizeof(dest.sin_addr.s_addr));
...
unique_ptr<RawSocket> WindowsBootstrapper::get_socket(const sockaddr& bind_to) {
	return unique_ptr<RawSocket>(new WindowsSocket(bind_to));
}
