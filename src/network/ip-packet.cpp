...
IpPacket::IpPacket(vector<uint8_t>& packet) {
    ...
    dscp = packet.at(1);
    packet_length = ntohs(packet.begin() + 2);
    id = ntohs(pac_iter + 4);
    ...
    payload = vector<uint8_t>(pac_iter + header_length, packet.end());
}
...
vector<uint8_t> IpPacket::get_payload() { return payload; }

shared_ptr<Protocol> IpPacket::get_inner_protocol() {
    if (inner_protocol == nullptr) {
        if (protocol == IP_UDP)
            inner_protocol = make_shared<UdpMessage>(payload);
        else if (protocol == IP_TCP)
            inner_protocol = make_shared<TcpSegment>(payload);
    }
